...
class CollectionStore {
  ...
  */
  ...
  listCollections(filters, options) {
    ...
    if(!options.columns) options.columns = ['collection_id', 'name', 'created', 'edited', 'author', 'description', 'private', 'public', 'app_id', "(SELECT COUNT(*) from records WHERE records.collections @> ARRAY[\"collection_id\"::varchar] AND training=FALSE) AS count"]
    ...
    return new Promise(function(resolve, reject) {
      ...
      this.db.collections.count(query, function(err, count) {
        ...
        this.db.collections.find(
          query,
          {columns: ['collection_id', 'name', 'created', 'edited', 'author', 'description', 'private', 'public', 'app_id', "(SELECT COUNT(*) from records WHERE \"collection_id\"=ANY(records.collections)) AS count"], limit: 10},
